...
import com.example.myapplication.databinding.QuickieOverlayViewBinding
...
internal class QROverlayView @JvmOverloads constructor(
  ...
  private val grayColor = ContextCompat.getColor(context, R.color.color_gray)
  ...
  fun setGalleryVisibilityAndOnClick(visible: Boolean, action: () -> Unit = {}) {
    binding.galleryImageView.visibility = if (visible) View.VISIBLE else View.GONE
    binding.galleryImageView.setOnClickListener { action() }
  ...
  private fun getAccentColor(): Int {
    return TypedValue().let {
      if (context.theme.resolveAttribute(android.R.attr.colorAccent, it, true)) {
        ...
        ContextCompat.getColor(context, R.color.color_352F44)
      }
    }
  }
